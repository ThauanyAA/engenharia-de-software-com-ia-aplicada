    ...
    return async (state: GraphState): Promise<Partial<GraphState>> => {
        ...
        try {
            ...
            if (!parsed.intent || !parsed.fileType) {
                ...
                throw new Error('Invalid intent data received');
            }

            parsed.fileName = `report.${parsed.fileType}`;

            console.log('✅ Intent result:', parsed.intent);
            console.log('✅ File type result:', parsed.fileType);
            console.log('✅ File name result:', parsed.fileName);
